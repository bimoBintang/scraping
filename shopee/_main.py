...
Usage:
    # Login (FIRST TIME or when cookies expire)
    python shopee_main.py login
    
    # Add product to wishlist (instant, no browser needed)
    python shopee_main.py add "https://shopee.co.id/product/123/456"
    python shopee_main.py add "https://shopee.co.id/product/123/456" --target 130000
    
    # List wishlist
    python shopee_main.py list
    
    # Check prices now (uses browser)
    ...
    # Search products (uses browser)
    python shopee_main.py search "laptop gaming" --limit 10
    
    # Remove from wishlist
    python shopee_main.py remove 12345678
    
    # Export data
    python shopee_main.py export wishlist --format csv
"""
...
from shopee import WishlistManager, ShopeeExporter
from shopee.models import WishlistItem
...
def check_cookies_status(cookies_path: str):
    ...
    if not cookies_path or not os.path.exists(cookies_path):
        print("⚠️  Cookies belum tersedia. Jalankan: python shopee_main.py login")
        return False
    ...
    if age_hours > 48:
        ...
        print("   Refresh dengan: python shopee_main.py login")
        return False
    elif age_hours > 24:
...
async def cmd_add(args):
    ...
    print(f"\n💡 Jalankan 'python shopee_main.py check' untuk cek harga terbaru")
...
async def cmd_list(args):
    ...
    if not items:
        ...
        print("   python shopee_main.py add <URL>")
        return
...
async def cmd_report(args):
    """Show price report"""
    from shopee.price_tracker import PriceTracker
...
async def cmd_search(args):
    ...
    if not products:
        print("❌ Tidak ada hasil. Coba 'python shopee_main.py login' untuk refresh cookies.")
        return
...
def main():
    ...
    if not args.command:
        print_banner()
        parser.print_help()
        print("\n💡 Mulai dengan: python shopee_main.py login")
        return
